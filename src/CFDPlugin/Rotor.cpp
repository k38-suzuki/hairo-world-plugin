    ...
    : SceneDevice(device)
{
    ...
    if(!scene) {
        ...
        scene->setTranslation(Vector3(0.0, 0.0, 0.025));
        scene->setRotation(rotFromRpy(radian(Vector3(90.0, 0.0, 0.0))));
    }
...
void SceneRotor::onStateChanged()
{
...
struct RotorRegistration
{
    RotorRegistration() {
        ...
        StdBodyWriter::registerDeviceWriter<Rotor>(
                    "Rotor",
